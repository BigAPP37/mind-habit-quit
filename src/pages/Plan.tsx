...
import { Target, Zap, MessageCircle, Calendar } from 'lucide-react';
...
export default function Plan() {
  ...
  return (
    <Layout>
      <div className="max-w-lg mx-auto px-4 pt-4 sm:pt-6 pb-4 space-y-4">
        <h1 className="text-xl sm:text-2xl font-serif font-bold text-foreground">Mi Plan</h1>
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-none">
          {tabs.map(t => (
            <button
              ...
              className={`px-3 py-2 rounded-lg text-xs font-medium whitespace-nowrap transition-all min-h-[40px] ${
        ...
        {tab === 'plan' && profile && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3 sm:space-y-4">
            <div className="p-3.5 sm:p-4 rounded-xl bg-card shadow-card space-y-2.5 sm:space-y-3">
            ...
            <div className="p-3.5 sm:p-4 rounded-xl bg-card shadow-card">
              <h3 className="font-semibold text-foreground text-sm mb-2">Mis disparadores</h3>
              <div className="flex flex-wrap gap-1.5">
                {profile.triggers.map(t => (
                  <span key={t} className="px-2.5 py-1 rounded-full bg-emergency/10 text-emergency text-xs">{t}</span>
                ))}
              </div>
            </div>
            ...
            <div className="p-3.5 sm:p-4 rounded-xl bg-card shadow-card">
              ...
              <div className="flex flex-wrap gap-1.5">
                {profile.mainReasons.map(r => (
                  <span key={r} className="px-2.5 py-1 rounded-full bg-success/10 text-success text-xs">{r}</span>
                ))}
              </div>
            </div>
          </motion.div>
        ...
        {tab === 'ifthen' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
            <p className="text-xs text-muted-foreground">Reglas automáticas: cuando ocurra X, haz Y.</p>
            {rules.map(r => (
              <div key={r.id} className={`p-3 rounded-xl border transition-all ${r.active ? 'bg-card shadow-card border-transparent' : 'bg-muted/50 border-border opacity-60'}`}>
                <div className="flex items-start gap-3">
                  <button onClick={() => toggleRule(r.id)}
                    className={`mt-0.5 w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all min-w-[24px] ${r.active ? 'bg-primary border-primary' : 'border-border'}`}
                  >
              ...
              <div key={ar.trigger} className="p-3 rounded-xl bg-card shadow-card">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {ar.alternatives.map(a => (
                    <span key={a} className="px-2.5 py-1 rounded-full bg-secondary text-secondary-foreground text-xs">{a}</span>
                  ))}
                </div>
              </div>
            ))}
        ...
        {tab === 'frases' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
            <p className="text-xs text-muted-foreground">Frases de autosugestión. Repítelas en voz baja, con convicción.</p>
            ...
            <p className="text-xs text-muted-foreground text-center pt-2">
              ⚠️ Técnica de enfoque y visualización. Úsalo como complemento.
            </p>
          </motion.div>
        )}
      </div>
